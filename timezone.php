	...
	{ 
		...
		foreach($element as $key => $sub_tree)
		{
			...
			else
			{
				...
				if ($hour_place==$hour)  // If the user entered hour matches with the current city hour go ahead to match minutes 
				{
					$minute_difference = $minute_place - $minute ;    // find the minute difference 
					// echo $minute_difference."<br>";
					if ($minute_difference > -15 && $minute_difference < 15)  // compensate for server call less 
